(() => {
  ...
  var require_app = __commonJS({
    "src/app.js"() {
      ...
      var CONFIG = window.__NOVACALL_CONFIG__ || {
      ...
      var btnGenerateRoom = document.getElementById("btnGenerateRoom");
      var btnBackToJoin = document.getElementById("btnBackToJoin");
      ...
      function startCallTimer() {
        ...
        callTimer = setInterval(() => {
          const s = Math.floor((Date.now() - callStartTime) / 1e3);
      ...
      function enterCallPage(roomId2) {
        joinPage?.classList.remove("page--active");
        callPage?.classList.add("page--active");
        if (roomBadge) roomBadge.textContent = roomId2 || "-";
        startCallTimer();
      }
      ...
      function ensurePeerTile(peerId2) {
        ...
        if (!peerId2) return null;
        if (peerTiles.has(peerId2)) return peerTiles.get(peerId2);
        const el = document.createElement("div");
        el.className = "thumb-card";
        el.dataset.peerId = peerId2;
      ...
      function removePeerTile(peerId2) {
        ...
        peerTiles.delete(peerId2);
        if (pinnedPeerId === peerId2) {
          pinnedPeerId = null;
      ...
      function pinPeer(peerId2) {
        const tile = peerTiles.get(peerId2);
        if (!tile || !tile.videoEl?.srcObject) return;
        pinnedPeerId = peerId2;
